...
Server::Server(int domain, int service, int protocol, u_short port,
               u_long interface, int numconn) {
  num_workers = numconn;
  server_socket =
      std::make_unique<Socket>(domain, service, protocol, port, interface);
  server_socket->bind_socket();
  server_socket->listen_socket(numconn);
}
int Server::accept_connection() { return server_socket->accept_connection(); }

void Server::handle_connection(int client_socket_fd) {
  std::string buffer(30000, '\0');

  std::cout << "AAADDRess of string buffer: " << &buffer
            << " and size: " << buffer.size() << std::endl;

  // string.c_str : Returns a pointer a null-terminated C-style string. seems
  // like returning const char * so it cannot be modified string.data:  Does not
  // require nut termination and returns char *
  ...
  std::cout << "The working thread is with ID: " << std::this_thread::get_id()
            << " with socket ID: " << client_socket_fd << std::endl;
...
bool Server::get_interrupt_stat() { return interrupted; }
...
void Server::launch_server() {
  ...
  while (!get_interrupt_stat()) {
    ...
    // tp.submit(std::packaged_task<void()>([this, conn](){return
